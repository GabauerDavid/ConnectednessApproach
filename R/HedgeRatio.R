...
#' @export
HedgeRatio = function (x, H, method = c("cumsum", "cumprod"), 
                       statistics = c("Fisher", "Bartlett", "Fligner-Killeen", 
                                      "Levene", "Brown-Forsythe"), digit = 2) 
{
  ...
  HR = array(NA, c(k, k, t), dimnames = list(NAMES, NAMES, 
                                             date))
  for (i in 1:k) {
    for (j in 1:k) {
      HR[i, j, ] = H[i, j, ]/H[j, j, ]
  ...
  for (i in 1:k) {
    for (j in 1:k) {
      portfolio_return[i, j, ] = as.numeric(x[, i] - HR[i, 
                                                        j, ] * x[, j])
      HE[j, i] = 1 - var(portfolio_return[i, j, ])/var(x[, 
                                                         i])
      df = rbind(data.frame(val = portfolio_return[i, j, 
      ...
      if (method == "cumsum") {
        cumulative_portfolio_return[i, j, ] = cumsum(portfolio_return[i, 
                                                                      j, ])
      }
      else if (method == "cumprod") {
        cumulative_portfolio_return[i, j, ] = cumprod(1 + 
                                                        portfolio_return[i, j, ]) - 1
      }
    }
  }
