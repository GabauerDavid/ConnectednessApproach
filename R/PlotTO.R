...
#' @export
PlotTO = function(dca, ca=NULL, path=NULL, ylim=c(NULL, NULL), width=10, height=7, mfcol=NULL, ...) {
  if (!is.null(path)) {
    if (!dir.exists(path)) {
      dir.create(path)
    }
  ...
  on.exit(par(oldpar)) 
  if (!is.null(path)) pdf(file=paste0(path, "/TO.pdf"), width=width, height=height)
  ...
  if (length(dim(dca$NET))>2) {
    for (i in 1:k) {
      ...
      title(paste0("From ", NAMES[i], "TO all others"), adj=0)
      grid(NA, NULL, lty=2)
      for (j in 1:ncol(x_)) {
